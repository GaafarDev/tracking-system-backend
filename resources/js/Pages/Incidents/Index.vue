...
import { ExclamationTriangleIcon, ExclamationCircleIcon, CheckCircleIcon, ClockIcon } from '@heroicons/vue/24/outline';
...
function resolveIncident(incidentId) {
    axios.post(`/incidents/${incidentId}/resolve`)
        .then(() => {
                                ...
                                <tr v-for="incident in props.incidents.data" :key="incident.id" 
                                    ...
                                    <td class="px-6 py-5 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end space-x-2">
                                            <button v-if="!['resolved', 'closed'].includes(incident.status)" 
...
export default {
    methods: {
        resolveIncident(incidentId) {
            // No confirmation popup - direct action like SOS alerts
            axios.post(route('incidents.resolve', incidentId))
                .then(() => {
                    router.reload({ only: ['incidents'] });
                })
                .catch(error => {
